...
class web_action(object):
    ...
    def web_logout(self, name):
        ...
        self.browser.find_element_by_xpath(self.element._get_logout_element(name)).click()
        self.browser.quit()
    ...
    def _features_page(self, path):
        ...
        if len(path) >= 2 :
            self.browser.find_element_by_xpath(self.element._get_classification_element(path[0])).click()
            self.browser.find_element_by_xpath(self.element._get_module_element(path[1])).click()
            if len(path) >= 3:
                self.browser.find_element_by_xpath(self.element._get_function_element(path[2])).click()
                if len(path) == 4:
                    time.sleep(1)
                    self.browser.switch_to_frame("Main_con")
                    self.browser.find_element_by_xpath(self.element._get_header_element(path[3])).click()
        else :
    ...
    def _iframe_input_box(self, name, path, value):
        ...
        element = self.browser.find_element_by_xpath(self.element._get_iframe_input_file_element(name))
